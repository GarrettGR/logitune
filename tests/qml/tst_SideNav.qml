...
Item {
    ...
    TestCase {
        ...
        function test_clickSettingsChangesPage() {
            ...
            // Header ≈ 53px. Items: 0→53, 1→110, 2→167, 3→224, 4→281
            // Settings (index 4) center ≈ 301
            mouseClick(nav, 100, 301)
        ...
        function test_disabledFlowDoesNotChangePage() {
            waitForRendering(nav)

            // flow (index 3) center ≈ 244
            mouseClick(nav, 100, 244)

            compare(nav.currentPage, "buttons", "clicking disabled flow item should not change page")
        }
    }
}
